...
export function parseCertificateToJson(certificate: string): Promise<Record<never, never>> {
  return requestClient().post<Record<never, never>>('/parsePublicCert/parseCertificateToJson', {
    certificate,
  })
}
