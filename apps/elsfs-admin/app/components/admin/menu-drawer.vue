...
import { filterMenuTree, resolveMenuPath } from '~/utils/admin-menu'
            ...
            >
              ...
              <div class="grid grid-cols-2 gap-1 md:grid-cols-3">
                <div
                  ...
                  :key="child.id"
                  class="group flex items-center rounded-lg pr-1 transition-colors hover:bg-accent"
                >
                  <button
                    type="button"
                    class="flex min-w-0 flex-1 items-center gap-2 px-2.5 py-2 text-left text-sm text-foreground"
                  ...
                  <button
                    ...
                    :title="menuStore.isFavorite(child.id) ? t('admin.removeFavorite') : t('admin.addFavorite')"
                    @click="menuStore.toggleFavorite(child.id)"
                  >
                    <AppIcon
                      :name="menuStore.isFavorite(child.id) ? 'star-filled' : 'star'"
                      class="size-4"
                    />
                  </button>
                </div>
